...
public class ViewLecturers extends JFrame{
    ...
    void loadLecturers(){
        String[] columns = {"ID", "Name", "Email", "NIC", "DOB", "Education", "Department", "No", "Street", "City","Profile Picture", "Phone","Action"};
        DefaultTableModel model = new DefaultTableModel(columns,0);
        String query = "SELECT u.Profile_pic,l.* FROM LECTURER l JOIN user u ON l.Lec_id = u.User_id";
        ...
        try {
            ...
            while(rs.next()){
                Object row[]={
                        ...
                        "View", // view phone numbers lable
                        "Edit" //edit profile btn lable
                };
                model.addRow(row);
            }
            ...
            table.getColumn("Phone").setCellEditor(new DefaultCellEditor(new JCheckBox()) {
                private JButton btn = new JButton("📞 Contact");
                {
                    btn.addActionListener(e -> {
                        ...
                        fireEditingStopped();
                        String lecId = table.getValueAt(row, 0).toString();
                        new ViewPhonesGUI(lecId, "LECTURER_PHONE", "Lec_id").setVisible(true);
                    });
                }
            ...
            // Edit button editor - these values will be passed to EditLecturerGUI constructor
            table.getColumn("Action").setCellEditor(new DefaultCellEditor(new JCheckBox()) {
...

                {
                    btn.addActionListener(e -> {
                        ...
                        String education = table.getValueAt(row, 5).toString();
                        String dpt       = table.getValueAt(row, 6).toString();
                        String no        = table.getValueAt(row, 7).toString();
                        String street    = table.getValueAt(row, 8).toString();
                        String city      = table.getValueAt(row, 9).toString();
                        String pic       = table.getValueAt(row, 10) != null ? table.getValueAt(row, 10).toString() : "";

                        new EditLecturerGUI(lecId, name, email, nic, dob, education, dpt, no, street, city, pic, ViewLecturers.this).setVisible(true);
                    });
                }
